...
ShellRoot {
    ...
    readonly property var builtinShellConfig: ({
            ...
            bar: {
                position: "top",
                transparent: false,
                centerAnchor: "itterum-shell.clock",
                layout: {
                    left: [
                        {
                            id: "itterum-shell.menu"
                        },
                        {
                            id: "itterum-shell.workspaces"
                        }
                    ],
                    center: [
                        {
                            id: "itterum-shell.clock",
                            format: "dddd HH:mm"
                        }
                    ],
                    right: [
                        {
                            id: "itterum-shell.audio"
                        }
                    ]
                }
            },
        ...
        if (userText.trim()) {
            try {
                ...
                if (Util.isPlainObject(parsed) && parsed.version === 1)
                    user = parsed;
                else if (Util.isPlainObject(parsed))
                    console.warn("shell.json missing version: 1, using defaults");
            } catch (e) {
    ...
    onBarConfigChanged: if (bar && "barConfig" in bar)
        bar.barConfig = shell.barConfig
    FileView {
        id: defaultsFile
    ...
    Component.onCompleted: {
        console.log("omarchy-shell paths", "omarchyPath=" + shell.omarchyPath, "shellDir=" + Quickshell.shellDir, "firstPartyPluginsDir=" + shell.firstPartyPluginsDir, "defaultsPath=" + shell.defaultsPath, "userConfigPath=" + shell.userConfigPath);
        pluginRegistry.firstPartyDir = shell.firstPartyPluginsDir;
        pluginRegistry.shellConfigProvider = function () {
            return shell.shellConfig;
        };
        pluginRegistry.shellConfigMutator = function (mutate) {
    ...
    onSelectedBarIdChanged: if (failedBarId !== "")
        failedBarId = ""
    ...
    function isBarOptionManifest(manifest) {
        return manifest && Array.isArray(manifest.kinds) && manifest.kinds.indexOf("bar") !== -1 && manifest.entryPoints && manifest.entryPoints.bar;
    }

    function barOptionAvailable(pluginId) {
        var id = String(pluginId || "");
        if (id === "" || id === shell.defaultBarId)
            return true;
    ...
    Loader {
        ...
        onActiveChanged: if (!active && shell.activeBarId !== shell.defaultBarId)
            shell.bar = null
    }
    ...
    function ensureService(pluginId) {
        ...
        var manifest = pluginRegistry && pluginRegistry.installedPlugins ? pluginRegistry.installedPlugins[key] : null;
        if (!manifest)
            return null;
        if (!Array.isArray(manifest.kinds) || manifest.kinds.indexOf("service") === -1)
            return null;
        if (!manifest.entryPoints || !manifest.entryPoints.service)
            return null;
        ...
        function finalize() {
            ...
            if ("omarchyPath" in inst)
                inst.omarchyPath = shell.omarchyPath;
            if ("shell" in inst)
                inst.shell = shell;
            if ("manifest" in inst)
                inst.manifest = manifest;
            if ("barWidgetRegistry" in inst)
                inst.barWidgetRegistry = shell.barWidgetRegistry;
            if ("pluginRegistry" in inst)
                inst.pluginRegistry = shell.pluginRegistry;
            var snext = ({});
            for (var sk in _services)
                snext[sk] = _services[sk];
            snext[key] = inst;
            _services = snext;
        }
    ...
    function _syncServices() {
        ...
        for (var id in plugins) {
            ...
            if (!m)
                continue;
            if (!Array.isArray(m.kinds) || m.kinds.indexOf("service") === -1)
                continue;
            if (!m.entryPoints || !m.entryPoints.service)
                continue;
    ...
    function updateEntryInline(moduleName, settings) {
        ...
        var copy = JSON.parse(JSON.stringify(shellConfig || builtinShellConfig));
        if (!Util.isPlainObject(copy.bar))
            copy.bar = {
                layout: {
                    left: [],
                    center: [],
        ...
        if (!Util.isPlainObject(copy.bar.layout))
            copy.bar.layout = {
                left: [],
                center: [],
                right: []
        ...
        if (!foundInLayout) {
            for (var j = 0; j < copy.plugins.length; j++) {
                if (copy.plugins[j] && copy.plugins[j].id === stripped) {
                    ...
                    for (var pk in settings)
                        if (pk !== "id")
                            pnext[pk] = settings[pk];
    ...
    function isBarWidgetPanelPlugin(pluginId) {
        ...
        if (!m || !Array.isArray(m.kinds))
            return false;
        if (m.kinds.indexOf("bar-widget") === -1)
            return false;
        ...
        for (var i = 0; i < loaderKinds.length; i++) {
            if (m.kinds.indexOf(loaderKinds[i]) !== -1)
                return false;
        }
        return true;
    }
    ...
    function summon(pluginId, payloadJson) {
        ...
        if (shell.isBarWidgetPanelPlugin(id)) {
            var summoned = shell.bar && typeof shell.bar.summonBarWidget === "function" && shell.bar.summonBarWidget(id);
            if (!summoned)
                console.warn("summon: no live bar widget for:", id);
            return summoned === true;
        }
        var next = ({});
        for (var k in openPanelIds)
            next[k] = openPanelIds[k];
        next[id] = true;
        openPanelIds = next;

        // Stash payload so the Loader.onLoaded handler can hand it to open().
        var pending = ({});
        for (var p in pendingPayloads)
            pending[p] = pendingPayloads[p].slice();
    ...
    function hide(pluginId) {
        ...
        if (!id)
            return false;
        if (shell.isBarWidgetPanelPlugin(id)) {
            var hidden = shell.bar && typeof shell.bar.hideBarWidget === "function" && shell.bar.hideBarWidget(id);
            if (!hidden)
                console.warn("hide: no live bar widget for:", id);
            return hidden === true;
        }
    ...
    function isPluginOpen(pluginId) {
        var id = shell.pluginRegistry.resolveEnabledId(pluginId);
        if (shell.isBarWidgetPanelPlugin(id)) {
            return shell.bar && typeof shell.bar.isBarWidgetOpen === "function" ? shell.bar.isBarWidgetOpen(id) : false;
        }
    ...
    function registerPanelLoader(pluginId, loader) {
        var next = ({});
        for (var k in panelLoaders)
            next[k] = panelLoaders[k];
    ...
    function deliverIfLoaded(pluginId) {
        ...
        var next = ({});
        for (var k in pendingPayloads)
            if (k !== pluginId)
                next[k] = pendingPayloads[k].slice();
    ...
    function computePanelEntries() {
        ...
        for (var id in plugins) {
            ...
            for (var i = 0; i < panelKinds.length; i++)
                if (m.kinds.indexOf(panelKinds[i]) !== -1) {
                    matched = true;
                    break;
                }
            if (!matched)
            ...
            if (!shell.pluginRegistry.isEnabled(id))
                continue;
            var kind = m.kinds.indexOf("panel") !== -1 ? "panel" : (m.kinds.indexOf("overlay") !== -1 ? "overlay" : "menu");
            out.push({
                id: id,
                manifest: m,
                kind: kind,
        ...
        delegate: QtObject {
            ...
            property Loader panelLoader: Loader {
                ...
                onLoaded: {
                    if (!item)
                        return;
                    if ("omarchyPath" in item)
                        item.omarchyPath = shell.omarchyPath;
                    if ("shell" in item)
                        item.shell = shell;
                    if ("manifest" in item)
                        item.manifest = panelEntry.manifest;
                    if ("barWidgetRegistry" in item)
                        item.barWidgetRegistry = shell.barWidgetRegistry;
                    if ("pluginRegistry" in item)
                        item.pluginRegistry = shell.pluginRegistry;
                    ...
                    // loaded.
                    if ("service" in item)
                        item.service = shell.serviceFor(panelEntry.pluginId);
    ...
    function syncPluginWidgets() {
        ...
        for (var pluginId in plugins) {
            ...
            if (!manifest || !manifest.kinds || manifest.kinds.indexOf("bar-widget") === -1)
                continue;
            if (!shell.pluginRegistry.isEnabled(pluginId))
                continue;
            ...
            if (existing && existing.url === url && !existing.component)
                continue;
        ...
        for (var i = 0; i < allIds.length; i++) {
            var id = allIds[i];
            if (!pluginWidgetComponents[id])
                continue;
            if (!seen[id]) {
                shell.barWidgetRegistry.unregister(id);
                var next = ({});
                for (var k in pluginWidgetComponents)
                    if (k !== id)
                        next[k] = pluginWidgetComponents[k];
    ...
    IpcHandler {
        ...
        function open(imageDirs: string, imageRowsB64: string, selectedImage: string, selectionFile: string, doneFile: string, showLabels: string, filterable: string): string {
        ...
        function preload(imageRowsB64: string, selectedImage: string, showLabels: string, filterable: string): string {
            var picker = shell.imagePickerItem();
            if (picker && typeof picker.preloadRows === "function") {
                picker.preloadRows(Util.decodeBase64(imageRowsB64), selectedImage, showLabels, filterable);
            }
            return "ok";
        }
    ...
    IpcHandler {
        ...
        function applyTheme(colorsB64: string, shellB64: string): string {
            ...
            var shellRaw = "";
            try {
                colorsRaw = Qt.atob(String(colorsB64 || ""));
            } catch (e) {
                colorsRaw = "";
            }
            try {
                shellRaw = Qt.atob(String(shellB64 || ""));
            } catch (e2) {
                shellRaw = "";
            }
        ...
        function listPlugins(): string {
            ...
            for (var id in plugins) {
                ...
                out.push({
                    ...
                    enabled: isBarOption ? active : (isBarWidget ? shell.pluginRegistry.inBar(id) : shell.pluginRegistry.isEnabled(id)),
                    active: active,
        ...
        function togglePanelAt(section: string, index: string): string {
            var id = shell.bar && typeof shell.bar.panelWidgetIdAt === "function" ? shell.bar.panelWidgetIdAt(section, index) : "";
            if (!id)
                return "unknown";
            shell.toggle(id, "{}");
            return id;
